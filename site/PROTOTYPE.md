...
The desktop card is 590×382px at 1024 wide and scales in three steps (×1.06, ×1.12, ×1.16) on larger screens, sitting slightly above centre. On phones the compact card is deliberately portrait (about 88% of the width, 1:1.42, capped at 372px wide and to the viewport height): the name stacks large in the lower half of the front, the back gives the index more room, and both faces are composed independently inside the same outer size. Expanding it drops the portrait constraint so the same sheet grows into the full-width surface; pointer transforms are disabled on touch. Nothing on the stage is larger than the stage (the ambient field is two stage-sized layers that crossfade, and the phone ground shadow is a zero-height box-shadow), so the compact card adds no scroll range in any browser; the stage clip is only a backstop. On phones the expanded surface still scrolls by touch, wheel and keyboard, but its scrollbar is hidden. The soft shadow beneath the card and a faint paper sheen are driven by CSS variables fed from the same spring values as the tilt, so they settle with it and never touch React state. The surrounding field brightens slightly on hover, widens on the back and grows with the card when it expands; with reduced motion all of this is static. Expanded mobile surfaces fill the available height with safe-area padding; typography, diagrams and content columns adapt vertically.
